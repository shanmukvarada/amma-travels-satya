...
import { LogOut, Activity, Car, FileText } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col md:flex-row pb-20 md:pb-0">
      ...
      <aside className="hidden md:flex w-64 bg-white border-r border-gray-200 flex-col shadow-sm z-10 sticky top-0 h-screen">
        ...
        <nav className="flex flex-col gap-1 px-4 flex-1">
          ...
          <Link href="/admin/vehicles" className={`px-4 py-2.5 flex items-center gap-3 rounded-lg font-medium transition-colors ${pathname === '/admin/vehicles' ? 'bg-red-50 text-red-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 flex justify-around items-center px-2 py-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] safe-area-pb">
        ...
        <Link href="/admin/vehicles" className={`flex flex-col items-center justify-center p-2 min-w-[70px] ${pathname === '/admin/vehicles' ? 'text-red-700 font-bold' : 'text-gray-500 font-medium'}`}>
          <div className={`p-1.5 rounded-full mb-1 ${pathname === '/admin/vehicles' ? 'bg-red-50' : ''}`}>
            <Car size={20} />
          </div>
