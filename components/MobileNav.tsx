...
import { Car, FileText, PhoneCall } from 'lucide-react';
...
export default function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 flex justify-around items-center px-2 py-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] safe-area-pb">
      ...
      <Link href="/status" className={`flex flex-col items-center justify-center p-2 min-w-[70px] ${pathname === '/status' ? 'text-red-700 font-bold' : 'text-gray-500 font-medium'}`}>
        <div className={`p-1.5 rounded-full mb-1 ${pathname === '/status' ? 'bg-red-50' : ''}`}>
          <FileText size={20} />
        </div>
